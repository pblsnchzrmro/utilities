...
file_name = 'precios_eoac_tarifa'
precios_eoac_tarifa_bp = Blueprint('precios_eoac_tarifa', __name__)

@precios_eoac_tarifa_bp.route('/PRECIOS_EOAC_TARIFA_DL', methods=['GET'])
def precios_eoac_tarifa_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/PRECIOS/eoac_tarifa.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Precios EOAC Tarifa dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=eoac_tarifa" + file_type}
