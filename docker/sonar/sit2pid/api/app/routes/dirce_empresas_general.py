...
dirce_empresas_general_bp = Blueprint('dirce_empresas_general', __name__)
...
@dirce_empresas_general_bp.route('/DIRCE_EMPRESAS_GENERAL_DL', methods=['GET'])
def dirce_empresas_general_dl():
    ...
    if (end_year and not start_year) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["CCAA"] = ccaa
        df = read_blob_file("/DIRCE/empresas_general.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Empresas general dataframe created, total rows {}...".format(df.shape[0]))
        ...
        header = {
            "Content-Disposition": "attachment; filename=dirce_empresas_general" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=dirce_empresas_general" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'dirce_empresas_general{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=dirce_empresas_general" + file_type
        return response
