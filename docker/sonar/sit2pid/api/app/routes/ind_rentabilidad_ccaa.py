...
file_name = 'ind_rentabilidad_ccaa'
ind_rentabilidad_ccaa_bp = Blueprint('ind_rentabilidad_ccaa', __name__)

@ind_rentabilidad_ccaa_bp.route('/IND_RENTABILIDAD_CCAA_DL', methods=['GET'])
def ind_rentabilidad_ccaa_dl():
    ...
    ccaa = "Total Nacional" if ccaa is None else ccaa
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["CCAA"] = ccaa if ccaa != "Total Nacional" else "Total Nacional"

        df = read_blob_file("/IND_RENTABILIDAD/ind_rentabilidad_ccaa.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_rentabilidad_ccaa" + file_type}
