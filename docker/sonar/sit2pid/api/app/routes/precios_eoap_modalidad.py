...
file_name = 'precios_eoap_modalidad'
precios_eoap_modalidad_bp = Blueprint('precios_eoap_modalidad', __name__)

@precios_eoap_modalidad_bp.route('/PRECIOS_EOAP_MODALIDAD_DL', methods=['GET'])
def precios_eoap_modalidad_dl():
    ...
    modality = request.args.get('Modalidad')
    file_type = request.args.get('Tipo de fichero')

    modality = "Total apartamentos" if modality is None else modality
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, modality))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["MODALIDAD"] = modality if modality != "Total apartamentos" else "Total apartamentos"

        df = read_blob_file("/PRECIOS/eoap_modalidad.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Precios EOAP Modalidad dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=eoap_modalidad" + file_type}
