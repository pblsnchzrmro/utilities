...
file_name = 'turismo_emisor_mun_pais'
turismo_emisor_mun_pais_bp = Blueprint('turismo_emisor_mun_pais', __name__)

@turismo_emisor_mun_pais_bp.route('/TURISMO_EMISOR_MUN_PAIS_DL', methods=['GET'])
def turismo_emisor_mun_pais_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/TELEFONIA_TURISMO/turismo_emisor_mun_pais.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Telefonia turismo dataframe created, total rows {}...".format(df.shape[0]))

        df = df.sort_values(by=["AÑO", "MES", "MUNICIPIO_ORIGEN", "CONTINENTE_DESTINO", "PAIS_DESTINO"])
        ...
        header = {
            "Content-Disposition": "attachment; filename=turismo_emisor_mun_pais" + file_type}
        # header = {"Access-Control-Allow-Origin": "*","Access-Control-Expose-Headers": "content-disposition","Content-Disposition": "attachment; filename=turismo_emisor_mun_pais" + file_type}
