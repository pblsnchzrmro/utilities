...
cst_empresas_industrias_turisticas_bp = Blueprint('cst_empresas_industrias_turisticas', __name__)
...
@cst_empresas_industrias_turisticas_bp.route('/CST_EMPRESAS_INDUSTRIAS_TURISTICAS_DL', methods=['GET'])
def cst_empresas_industrias_turisticas_dl():
    ...
    activity = request.args.get('Actividad')
    employees = request.args.get('Asalariados Empresas')
    file_type = request.args.get('Tipo de fichero')

    activity = "Todos" if activity is None else activity
    employees = "Todos" if employees is None else employees
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, activity, employees))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if activity != "Todos" and not "Todos" in activity:
            fields["ACTIVIDAD"] = activity
        if employees != "Todos" and not "Todos" in employees:
            fields["ASALARIADOS_EMPRESAS"] = employees

        df = read_blob_file("/CST/cst_empresa_industria_turisticas.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=cst_empresas_industrias_turisticas" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'cst_empresas_industrias_turisticas{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=cst_empresas_industrias_turisticas" + file_type
        return response
