...
file_name = 'seguridad_provincia'
seguridad_provincia_bp = Blueprint('seguridad_provincia', __name__)

@seguridad_provincia_bp.route('/SEGURIDAD_PROVINCIA_DL', methods=['GET'])
def seguridad_provincia_dl():
    ...
    logging.info(provincia)
    provincia = "Todos" if provincia is None else provincia
    file_type = ".xlsx" if file_type is None else file_type                          

    logging.info("Parameters: {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, provincia))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/POLICIA/policia_provincia.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Policia dataframe created, total rows {}...".format(df.shape[0]))
        
        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=seguridad_provincia" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=seguridad_provincia" + file_type}
