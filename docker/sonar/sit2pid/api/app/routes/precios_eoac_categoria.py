...
file_name = 'precios_eoac_categoria'
precios_eoac_categoria_bp = Blueprint('precios_eoac_categoria', __name__)

@precios_eoac_categoria_bp.route('/PRECIOS_EOAC_CATEGORIA_DL', methods=['GET'])
def precios_eoac_categoria_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/PRECIOS/eoac_categoria.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Precios EOAC Categoria dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=eoac_categoria" + file_type}
