...
file_name = 'vivienda_turistica_mun'
vivienda_turistica_mun_bp = Blueprint('vivienda_turistica_mun', __name__)

@vivienda_turistica_mun_bp.route('/VIVIENDA_TURISTICA_MUN_DL', methods=['GET'])
def vivienda_turistica_mun_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, province))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/VIVIENDA_TURISTICA/vut_mun.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Vut municipio dataframe created, total rows {}...".format(df.shape[0]))
        
        df = df[["AÑO", "MES", "PROVINCIA", "MUNICIPIO", "VIVIENDAS_TURISTICAS", "PLAZAS", "PLAZAS_POR_VIVIENDA_TURISTICA", "PORCENTAJE_VIVIENDAS_TURISTICAS"]]

        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=vut_mun" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=vut_mun" + file_type}
