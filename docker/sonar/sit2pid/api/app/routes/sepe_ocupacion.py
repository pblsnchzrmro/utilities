...
file_name = 'sepe_ocupacion'
sepe_ocupacion_bp = Blueprint('sepe_ocupacion', __name__)

@sepe_ocupacion_bp.route('/SEPE_OCUPACION_DL', methods=['GET'])
def sepe_ocupacion_dl():
    ...
    # ocupation = request.args.get('Ocupación')
    ...
    # ocupation = "Todos" if ocupation is None else ocupation
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        #     return Response(
        #         "You must select a value for ocupation!",
        #         status=500
        #     )
        if start_month:
        ...
        # fields["OCUPACION"] = ocupation

        df = read_blob_file("/SEPE/sepe_ocupacion.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=sepe_ocupacion" + file_type}
