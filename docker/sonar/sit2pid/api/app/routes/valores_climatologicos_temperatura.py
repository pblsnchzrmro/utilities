...
file_name = 'valores_climatologicos_temperatura'
valores_climatologicos_temperatura_bp = Blueprint('valores_climatologicos_temperatura', __name__)

@valores_climatologicos_temperatura_bp.route('/VALORES_CLIMATOLOGICOS_TEMPERATURA_DL', methods=['GET'])
def valores_climatologicos_temperatura_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        file_type = ".csv"
        df = read_blob_file("/AEMET/temperatura.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Temperatura dataframe created, total rows {}...".format(df.shape[0]))


        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=temperatura" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=temperatura" + file_type}
