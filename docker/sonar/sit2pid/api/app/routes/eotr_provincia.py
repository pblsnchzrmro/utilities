...
file_name = 'eotr_provincia'
eotr_provincia_bp = Blueprint('eotr_provincia', __name__)

@eotr_provincia_bp.route('/EOTR_PROVINCIA_DL', methods=['GET'])
def eotr_provincia_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EOTR/eotr_provincia.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=eotr_provincia" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eotr_provincia" + file_type}
