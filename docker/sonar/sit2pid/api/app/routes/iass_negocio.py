...
file_name = 'iass_negocio'
iass_negocio_bp = Blueprint('iass_negocio', __name__)

@iass_negocio_bp.route('/IASS_NEGOCIO_DL', methods=['GET'])
def iass_negocio_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/IASS/iass_negocio.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=iass_negocio" + file_type}
