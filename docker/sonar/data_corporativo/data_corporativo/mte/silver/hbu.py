...
df = spark.read.table("operaciones.bronze_mte.hBU")
...
df = df.withColumn("EsSector", when(col("EsSector") == True, 1).otherwise(0))
...
df = df.withColumn("hMA", when(col("hMA") == "", None).otherwise(col("hMA")))
...
# Crear tabla sector a partir de columna EsSector
df_sector = df.filter(col("EsSector") == 1).select("Nombre","FECHA_CARGA").distinct()
...
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('operaciones.silver_mte.hBU')

df_sector.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('operaciones.silver_mte.Sector')

# Save to PEOPLE catalog
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('people.silver_mte.hBU')

df_sector.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('people.silver_mte.Sector')