...
cst_locales_industrias_turisticas_bp = Blueprint('cst_locales_industrias_turisticas', __name__)
...
@cst_locales_industrias_turisticas_bp.route('/CST_LOCALES_INDUSTRIAS_TURISTICAS_DL', methods=['GET'])
def cst_locales_industrias_turisticas_dl():
    ...
    employees = request.args.get('Asalariados locales')
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        if employees != "Todos" and not "Todos" in employees:
            fields["ASALARIADOS_LOCALES"] = employees

        df = read_blob_file("/CST/cst_locales_industria_turisticas.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=cst_locales_industrias_turisticas" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'cst_locales_industrias_turisticas{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=cst_locales_industrias_turisticas" + file_type
        return response
