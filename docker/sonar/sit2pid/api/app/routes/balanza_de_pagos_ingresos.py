import logging
from flask import Blueprint, request, send_file, Response, make_response, make_response
from io import BytesIO
from common.blob_connection import read_blob_file, get_df_bytes_xlsx, get_df_bytes_csv

balanza_de_pagos_ingresos_bp = Blueprint('balanza_de_pagos_ingresos', __name__)
...
@balanza_de_pagos_ingresos_bp.route('/BALANZA_DE_PAGOS_INGRESOS_DL', methods=['GET'])
def balanza_de_pagos_ingresos_dl():
    ...
    zone = "TODO" if zone is None else zone
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, zone))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if zone != "TODO" and not "TODO" in zone:
            fields["DESC_ZONA"] = zone

        df = read_blob_file("/BP/bp_ingresos_zonas.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=bp_ingresos_zonas" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'bp_ingresos_zonas{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=bp_ingresos_zonas" + file_type
        return response
