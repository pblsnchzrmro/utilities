...
file_name = 'epa_economia_servicios_turismo'
epa_economia_servicios_turismo_bp = Blueprint('epa_economia_servicios_turismo', __name__)

@epa_economia_servicios_turismo_bp.route('/EPA_ECONOMIA_SERVICIOS_TURISMO_DL', methods=['GET'])
def epa_economia_servicios_turismo_dl():
    ...
    ocupacion = request.args.get('Ocupación')
    file_type = request.args.get('Tipo de fichero')
    
    ocupacion = "Todos" if ocupacion is None else ocupacion
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, ocupacion))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields["OCUPACION"] = ocupacion
       
        df = read_blob_file("/EPA/epa_economia_servicios_turismo.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=epa_economia_servicios_turismo" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=epa_economia_servicios_turismo" + file_type}
