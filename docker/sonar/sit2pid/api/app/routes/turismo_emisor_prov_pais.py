...
file_name = 'turismo_emisor_prov_pais'
turismo_emisor_prov_pais_bp = Blueprint('turismo_emisor_prov_pais', __name__)

@turismo_emisor_prov_pais_bp.route('/TURISMO_EMISOR_PROV_PAIS_DL', methods=['GET'])
def turismo_emisor_prov_pais_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["PROVINCIA_ORIGEN"] = provincia

        df = read_blob_file("/TELEFONIA_TURISMO/turismo_emisor_provincia_pais.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Telefonia turismo dataframe created, total rows {}...".format(df.shape[0]))

        df = df.sort_values(by=["AÑO", "MES", "PROVINCIA_ORIGEN", "CONTINENTE_DESTINO", "PAIS_DESTINO"])
        df  = df[["AÑO", "MES", "PROVINCIA_ORIGEN", "CONTINENTE_DESTINO", "PAIS_DESTINO", "TURISTAS", "PERNOCTACIONES", "ESTANCIA_MEDIA"]]

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        
        header = {
            "Content-Disposition": "attachment; filename=turismo_emisor_provincia_pais" + file_type}
        # header = {"Access-Control-Allow-Origin": "*","Access-Control-Expose-Headers": "content-disposition","Content-Disposition": "attachment; filename=turismo_emisor_provincia_pais" + file_type}
