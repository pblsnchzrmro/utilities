...
file_name = 'seguridad_ccaa'
seguridad_ccaa_bp = Blueprint('seguridad_ccaa', __name__)

@seguridad_ccaa_bp.route('/SEGURIDAD_CCAA_DL', methods=['GET'])
def seguridad_ccaa_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/POLICIA/policia_ccaa.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=seguridad_ccaa" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=seguridad_ccaa" + file_type}
