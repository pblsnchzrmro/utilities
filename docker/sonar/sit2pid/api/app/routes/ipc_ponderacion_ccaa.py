...
file_name = 'ipc_ponderacion_ccaa'
ipc_ponderacion_ccaa_bp = Blueprint('ipc_ponderacion_ccaa', __name__)

@ipc_ponderacion_ccaa_bp.route('/IPC_PONDERACION_CCAA_DL', methods=['GET'])
def ipc_ponderacion_ccaa_dl():
    ...
    rubrics = "Todos" if rubrics is None else rubrics
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        if ccaa != "Nacional" and not "Nacional" in ccaa:
            fields["CCAA"] = ccaa
        if rubrics != "Todos" and not "Todos" in rubrics:
            fields["RUBRICAS"] = rubrics

        df = read_blob_file("/IPC/ipc_ponderacion_ccaa.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ipc_ponderacion_ccaa" + file_type}
