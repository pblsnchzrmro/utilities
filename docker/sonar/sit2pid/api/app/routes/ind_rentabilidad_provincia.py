...
file_name = 'ind_rentabilidad_provincia'
ind_rentabilidad_provincia_bp = Blueprint('ind_rentabilidad_provincia', __name__)

@ind_rentabilidad_provincia_bp.route('/IND_RENTABILIDAD_PROVINCIA_DL', methods=['GET'])
def ind_rentabilidad_provincia_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/IND_RENTABILIDAD/ind_rentabilidad_provincia.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_rentabilidad_provincia" + file_type}
