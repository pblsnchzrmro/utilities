...
eoh_punt_tur_bp = Blueprint('eoh_punt_tur', __name__)
...
@eoh_punt_tur_bp.route('/EOH_PUNT_TUR_DL', methods=['GET'])
def eoh_punt_tur_dl():
    ...
    turistic_point = request.args.get('Punto turístico')
    ...
    turistic_point = "Todos" if turistic_point is None else turistic_point
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, turistic_point, residence, province))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["PUNTO_TURISTICO"] = turistic_point
        fields["LUGAR_RESIDENCIA"] = residence
        fields["PROVINCIA"] = province

        df = read_blob_file("/EOH/eoh_punt_tur.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("EOH dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=eoh_punt_tur" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eoh_punt_tur" + file_type} 
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'eoh_punt_tur{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=eoh_punt_tur" + file_type
        return response
