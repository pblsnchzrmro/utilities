...
df = spark.read.table("operaciones.bronze_mte.hGR")
...
df = df.withColumn("hGZ", when(col("hGZ") == "", None).otherwise(col("hGZ")))
...
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('operaciones.silver_mte.hGR')
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('people.silver_mte.hGR')