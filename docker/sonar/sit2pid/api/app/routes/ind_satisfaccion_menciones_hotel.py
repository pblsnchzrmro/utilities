...
file_name = 'ind_satisfaccion_menciones_hotel'
ind_satisfaccion_menciones_hotel_bp = Blueprint('ind_satisfaccion_menciones_hotel', __name__)

@ind_satisfaccion_menciones_hotel_bp.route('/IND_SATISFACCION_MENCIONES_HOTEL_DL', methods=['GET'])
def ind_satisfaccion_menciones_hotel_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/MABRIAN/mabrian_alojamientos.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_satisfaccion_menciones_hotel" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=mabrian_alojamientos" + file_type}
