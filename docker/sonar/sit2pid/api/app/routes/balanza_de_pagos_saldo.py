...
balanza_de_pagos_saldo_bp = Blueprint('balanza_de_pagos_saldo', __name__)
...
@balanza_de_pagos_saldo_bp.route('/BALANZA_DE_PAGOS_SALDO_DL', methods=['GET'])
def balanza_de_pagos_saldo_dl():
    ...
    balance = request.args.get('Saldo')
    file_type = request.args.get('Tipo de fichero')

    balance = "Saldo" if balance is None else balance
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, balance))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if balance != "Saldo" and not "Saldo" in balance:
            fields["DESC_SALDO"] = balance if balance != "Turismo" else "Servicios"

        df = read_blob_file("/BP/bp_saldo.csv", fields,
                            start_year, start_month, end_year, end_month)
        
        logging.info(
            "BP dataframe created, total rows {}...".format(df.size))
        
        if balance != "Saldo" and not "Saldo" in balance:
           df["DESC_SALDO"].replace({"Servicios": "Turismo"}, inplace=True)

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=bp_saldo" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'bp_saldo{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=bp_saldo" + file_type
        return response
