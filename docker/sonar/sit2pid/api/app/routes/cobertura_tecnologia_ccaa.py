...
cobertura_tecnologia_ccaa_bp = Blueprint('cobertura_tecnologia_ccaa', __name__)
...
@cobertura_tecnologia_ccaa_bp.route('/COBERTURA_TECNOLOGIA_CCAA_DL', methods=['GET'])
def cobertura_tecnologia_ccaa_dl():
    ...
    if (end_year and not start_year) or not file_type in [".csv", ".xlsx"]:
        ...
        logging.info(fields)
        df = read_blob_file("/COBERTURA/cobertura_ccaa_tecnologia.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=cobertura_ccaa_tecnologia" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'cobertura_ccaa_tecnologia{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=cobertura_ccaa_tecnologia" + file_type
        return response
