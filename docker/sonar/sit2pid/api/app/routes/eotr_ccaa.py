...
file_name = 'eotr_ccaa'
eotr_ccaa_bp = Blueprint('eotr_ccaa', __name__)

@eotr_ccaa_bp.route('/EOTR_CCAA_DL', methods=['GET'])
def eotr_ccaa_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EOTR/eotr_ccaa.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=eotr_ccaa" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eotr_ccaa" + file_type}
