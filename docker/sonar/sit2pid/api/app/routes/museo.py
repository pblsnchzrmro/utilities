...
@museo_bp.route('/MUSEO_DL', methods=['GET'])
def museo_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/MUSEO/museo_ccaa.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Museo dataframe created, total rows {}...".format(df.shape[0]))


        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=museo" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=museo" + file_type}
