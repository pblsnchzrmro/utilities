...
file_name = 'intensidad_trafico_mensual'
intensidad_trafico_mensual_bp = Blueprint('intensidad_trafico_mensual', __name__)

@intensidad_trafico_mensual_bp.route('/INTENSIDAD_TRAFICO_MENSUAL_DL', methods=['GET'])
def intensidad_trafico_mensual_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/INTENSIDAD_TRAFICO/imd_mensual.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Intensidad trafico mensual dataframe created, total rows {}...".format(df.shape[0]))
        
        df = df[["AÑO", "MES", "CCAA", "PROVINCIA", "CARRETERA", "TIPO_ESTACION", "ESTACION", "IMD_VEHICULO_LIGERO", "IMD_VEHICULO_PESADO", "IMD_VEHICULO_TOTAL"]]


        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=imd_mensual" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=imd_mensual" + file_type}
