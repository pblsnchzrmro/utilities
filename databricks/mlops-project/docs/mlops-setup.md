...
- You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-3842051123482980.0.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-3842051123482980.0.azuredatabricks.net#setting/accounts). If
...
Create and push a release branch called `realese` off of the `main` branch of the repository:
```
git checkout -b realese main
git push upstream realese
git checkout main
```
...
For future ML code changes, iterate against the `main` branch and regularly deploy your ML code from staging to production by merging code changes from the `main` branch into the `realese` branch.
