...
file_name = 'intensidad_trafico_anual'
intensidad_trafico_anual_bp = Blueprint('intensidad_trafico_anual', __name__)

@intensidad_trafico_anual_bp.route('/INTENSIDAD_TRAFICO_ANUAL_DL', methods=['GET'])
def intensidad_trafico_anual_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, ccaa, provincia))
    ...
    if (end_year and not start_year) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/INTENSIDAD_TRAFICO/imd_anual.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Intensidad trafico anual dataframe created, total rows {}...".format(df.shape[0]))

        df = df.drop(columns=['MES'])
        df = df[["AÑO", "CCAA", "PROVINCIA", "CARRETERA", "TIPO_ESTACION", "ESTACION", "IMD_VEHICULO_LIGERO", "IMD_VEHICULO_PESADO", "IMD_VEHICULO_TOTAL"]]

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=imd_anual" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=imd_anual" + file_type}
