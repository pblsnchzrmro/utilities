...
file_name = 'naturaleza_superficie_protegida'
naturaleza_superficie_protegida_bp = Blueprint('naturaleza_superficie_protegida', __name__)

@naturaleza_superficie_protegida_bp.route('/NATURALEZA_SUPERFICIE_PROTEGIDA_DL', methods=['GET'])
def naturaleza_superficie_protegida_dl():
    ...
    if (end_year and not start_year) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/PROTECCION_NATURALEZA/superficie_protegida.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {
            "Content-Disposition": "attachment; filename=superficie_protegida" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=superficie_protegida" + file_type}
