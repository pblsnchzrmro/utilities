...
balanza_de_pagos_cuenta_bienes_servicios_bp = Blueprint('balanza_de_pagos_cuenta_bienes_servicios', __name__)
...
@balanza_de_pagos_cuenta_bienes_servicios_bp.route('/BALANZA_DE_PAGOS_CUENTA_BIENES_SERVICIOS_DL', methods=['GET'])
def balanza_de_pagos_cuenta_bienes_servicios_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/BP/bp_cuenta_bienes_servicios.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=bp_cuenta_bienes_servicios" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'bp_cuenta_bienes_servicios{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=bp_cuenta_bienes_servicios" + file_type
        return response
