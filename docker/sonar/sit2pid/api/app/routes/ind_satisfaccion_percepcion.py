...
file_name = 'ind_satisfaccion_percepcion'
ind_satisfaccion_percepcion_bp = Blueprint('ind_satisfaccion_percepcion', __name__)

@ind_satisfaccion_percepcion_bp.route('/IND_SATISFACCION_PERCEPCION_DL', methods=['GET'])
def ind_satisfaccion_percepcion_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/MABRIAN/mabrian_indices.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_satisfaccion_percepcion" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=ind_satisfaccion_percepcion" + file_type}
