...
file_name = 'precios_eotr_modalidad'
precios_eotr_modalidad_bp = Blueprint('precios_eotr_modalidad', __name__)

@precios_eotr_modalidad_bp.route('/PRECIOS_EOTR_MODALIDAD_DL', methods=['GET'])
def precios_eotr_modalidad_dl():
    ...
    modality = "Total alquiler" if modality is None else modality
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["MODALIDAD"] = modality if modality != "Total alquiler" else "Total aquiler"

        logging.info("Parameters: {}, {}, {}, {}, {}...".format(
            start_month, start_year, end_month, end_year, modality))

        df = read_blob_file("/PRECIOS/eotr_modalidad.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Precios EOTR Modalidad dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=eotr_modalidad" + file_type}
