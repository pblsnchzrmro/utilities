...
valores_climatologicos_precipitaciones_bp = Blueprint('valores_climatologicos_precipitaciones', __name__)

@valores_climatologicos_precipitaciones_bp.route('/VALORES_CLIMATOLOGICOS_PRECIPITACIONES_DL', methods=['GET'])
def valores_climatologicos_precipitaciones_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/AEMET/precipitacion.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Precipitacion dataframe created, total rows {}...".format(df.shape[0]))


        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=precipitacion" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=precipitacion" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        return send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'valores_climatologicos_precipitaciones{file_type}', mimetype=mimetype)