...
file_name = 'ind_rentabilidad_punt_tur'
ind_rentabilidad_punt_tur_bp = Blueprint('ind_rentabilidad_punt_tur', __name__)

@ind_rentabilidad_punt_tur_bp.route('/IND_RENTABILIDAD_PUNT_TUR_DL', methods=['GET'])
def ind_rentabilidad_punt_tur_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, turistic_point))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if turistic_point != "Todos" and not "Todos" in turistic_point:
            fields["PUNTO_TURISTICO"] = turistic_point

        df = read_blob_file("/IND_RENTABILIDAD/ind_rentabilidad_punt_tur.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "IND_RENTABILIDAD dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=ind_rentabilidad_punt_tur" + file_type}
