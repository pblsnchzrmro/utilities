...
cobertura_velocidad_provincia_bp = Blueprint('cobertura_velocidad_provincia', __name__)
...
@cobertura_velocidad_provincia_bp.route('/COBERTURA_VELOCIDAD_PROVINCIA_DL', methods=['GET'])
def cobertura_velocidad_provincia_dl():
    ...
    if (end_year and not start_year) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/COBERTURA/cobertura_provincia_velocidad.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=cobertura_provincia_velocidad" + file_type}
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'cobertura_provincia_velocidad{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=cobertura_provincia_velocidad" + file_type
        return response
