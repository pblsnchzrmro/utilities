...
file_name = 'sepe_tipo_demandante'
sepe_tipo_demandante_bp = Blueprint('sepe_tipo_demandante', __name__)

@sepe_tipo_demandante_bp.route('/SEPE_TIPO_DEMANDANTE_DL', methods=['GET'])
def sepe_tipo_demandante_dl():
    ...
    # job_seeker_type = request.args.get('Tipo demandante')
    ...
    # job_seeker_type = "Todos" if job_seeker_type is None else job_seeker_type
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        # if job_seeker_type is None:
        #     return Response(
        #         "You must select a value for job_seeker_type!",
        #         status=500
        #     )
        if start_month:
        ...
        # fields["TIPO_DEMANDANTE"] = job_seeker_type

        df = read_blob_file("/SEPE/sepe_tipo_demandante.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "SEPE dataframe created, total rows {}...".format(df.size))

        df = df.astype({"NUM_TOT_DEMANDANTE": int})

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=sepe_tipo_demandante" + file_type}
