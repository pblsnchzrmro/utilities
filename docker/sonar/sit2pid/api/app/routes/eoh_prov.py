...
eoh_prov_bp = Blueprint('eoh_prov', __name__)

MIN_YEAR = 1999
MIN_MONTH = 1

@eoh_prov_bp.route('/EOH_PROV_DL', methods=['GET'])
def eoh_prov_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, ccaa, residence, province))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EOH/eoh_prov.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=eoh_prov" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eoh_prov" + file_type} 
        mimetype = "application/octet-stream" if file_type == ".csv" else "application/vnd.ms-excel"

        logging.info("Sending file response...")
        response = make_response(send_file(BytesIO(df_bytes), as_attachment=True, download_name=f'eoh_prov{file_type}', mimetype=mimetype))
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Expose-Headers'] = 'content-disposition'
        response.headers['Content-Disposition'] = "attachment; filename=eoh_prov" + file_type
        return response
