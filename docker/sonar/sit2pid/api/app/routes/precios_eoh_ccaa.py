...
file_name = 'precios_eoh_ccaa'
precios_eoh_ccaa_bp = Blueprint('precios_eoh_ccaa', __name__)

@precios_eoh_ccaa_bp.route('/PRECIOS_EOH_CCAA_DL', methods=['GET'])
def precios_eoh_ccaa_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/PRECIOS/eoh_ccaa.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Precios EOH CCAA dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=eoh_ccaa" + file_type}
