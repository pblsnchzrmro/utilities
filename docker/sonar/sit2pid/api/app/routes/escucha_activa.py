...
file_name = 'escucha_activa'
escucha_activa_bp = Blueprint('escucha_activa', __name__)

@escucha_activa_bp.route('/ESCUCHA_ACTIVA_DL', methods=['GET'])
def escucha_activa_dl():
    ...
    categoria = request.args.get('Idioma')
    endpoint = request.args.get('Medio')
    sentimiento = request.args.get('Sentimiento')
    file_type = request.args.get('Tipo de fichero')

    categoria = "Todos" if categoria is None else categoria
    endpoint = "Todos" if endpoint is None else endpoint
    sentimiento = "Todos" if sentimiento is None else sentimiento
    file_type = ".csv" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, categoria, endpoint, sentimiento))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if categoria != "Todos" and not "Todos" in categoria:
            fields["CATEGORIA"] = categoria
        if endpoint != "Todos" and not "Todos" in endpoint:
            fields["ENDPOINT"] = endpoint
        if sentimiento != "Todos" and not "Todos" in sentimiento:
            fields["SENTIMIENTO"] = sentimiento
        logging.info("reading blob")
    
    df = read_blob_file(f"/ESCUCHA_ACTIVA/busqueda.csv", fields, start_year, start_month, end_year, end_month)
    logging.info("Readed blob")
    logging.info("Escucha activa dataframe created, total rows {}...".format(df.shape[0]))

    df_bytes = get_df_bytes_csv(
        df) if file_type == ".csv" else get_df_bytes_xlsx(df)
    
    header = {
        "Content-Disposition": "attachment; filename=escucha_activa" + file_type}
