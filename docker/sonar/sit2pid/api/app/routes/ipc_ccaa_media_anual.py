...
file_name = 'ipc_ccaa_media_anual'
ipc_ccaa_media_anual_bp = Blueprint('ipc_ccaa_media_anual', __name__)

@ipc_ccaa_media_anual_bp.route('/IPC_CCAA_MEDIA_ANUAL_DL', methods=['GET'])
def ipc_ccaa_media_anual_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/IPC/ipc_ccaa_media_anual.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ipc_ccaa_media_anual_media_anual" + file_type}
