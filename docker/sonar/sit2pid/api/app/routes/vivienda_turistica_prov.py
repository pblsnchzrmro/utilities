...
file_name = 'vivienda_turistica_prov'
vivienda_turistica_prov_bp = Blueprint('vivienda_turistica_prov', __name__)

MIN_YEAR = 2020
MIN_MONTH = 1

@vivienda_turistica_prov_bp.route('/VIVIENDA_TURISTICA_PROV_DL', methods=['GET'])
def vivienda_turistica_prov_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, province))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/VIVIENDA_TURISTICA/vut_prov.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Vut provincia dataframe created, total rows {}...".format(df.shape[0]))


        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=vut_prov" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=vut_prov" + file_type}
