...
file_name = 'ind_rentabilidad_categoria'
ind_rentabilidad_categoria_bp = Blueprint('ind_rentabilidad_categoria', __name__)

@ind_rentabilidad_categoria_bp.route('/IND_RENTABILIDAD_CATEGORIA_DL', methods=['GET'])
def ind_rentabilidad_categoria_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/IND_RENTABILIDAD/ind_rentabilidad_categoria.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_rentabilidad_categoria" + file_type}
