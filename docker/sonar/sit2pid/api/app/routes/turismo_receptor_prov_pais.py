...
file_name = 'turismo_receptor_prov_pais'
turismo_receptor_prov_pais_bp = Blueprint('turismo_receptor_prov_pais', __name__)

@turismo_receptor_prov_pais_bp.route('/TURISMO_RECEPTOR_PROV_PAIS_DL', methods=['GET'])
def turismo_receptor_prov_pais_dl():
    ...
    provincia = request.args.get("Provincia", 'Todos')
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        if provincia is None:
            return Response(
                "You must select a value for provincia!",
                status=500
            )
        if start_month:
        ...
        fields = {}
        fields["PROVINCIA_DESTINO"] = provincia

        df = read_blob_file("/TELEFONIA_TURISMO/turismo_receptor_provincia_pais.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Telefonia turismo dataframe created, total rows {}...".format(df.shape[0]))

        df = df.sort_values(by=["AÑO", "MES", "PROVINCIA_DESTINO", "CONTINENTE_ORIGEN", "PAIS_ORIGEN"])
        df  = df[["AÑO", "MES", "PROVINCIA_DESTINO", "CONTINENTE_ORIGEN", "PAIS_ORIGEN", "TURISTAS", "PERNOCTACIONES", "ESTANCIA_MEDIA"]]

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        
        header = {
            "Content-Disposition": "attachment; filename=turismo_receptor_provincia_pais" + file_type}
        # header = {"Access-Control-Allow-Origin": "*","Access-Control-Expose-Headers": "content-disposition","Content-Disposition": "attachment; filename=turismo_receptor_provincia_pais" + file_type}
