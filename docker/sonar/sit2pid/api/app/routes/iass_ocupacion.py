...
file_name = 'iass_ocupacion'
iass_ocupacion_bp = Blueprint('iass_ocupacion', __name__)

@iass_ocupacion_bp.route('/IASS_OCUPACION_DL', methods=['GET'])
def iass_ocupacion_dl():
    ...
    sector = ["Índice General", "Hostelería", "Servicios de alojamiento", "Servicios de comidas y bebidas",
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, sector))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        fields["SECTORES"] = sector

        df = read_blob_file("/IASS/iass_ocupacion.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "IASS dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=iass_ocupacion" + file_type}
