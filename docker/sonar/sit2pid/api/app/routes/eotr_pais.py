...
file_name = 'eotr_pais'
eotr_pais_bp = Blueprint('eotr_pais', __name__)

@eotr_pais_bp.route('/EOTR_PAIS_DL', methods=['GET'])
def eotr_pais_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EOTR/eotr_pais.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=eotr_pais" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eotr_pais" + file_type}
