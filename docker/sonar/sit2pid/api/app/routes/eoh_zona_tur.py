...
file_name = 'eoh_zona_tur'
eoh_zona_tur_bp = Blueprint('eoh_zona_tur', __name__)


MIN_YEAR = 1999
MIN_MONTH = 1

@eoh_zona_tur_bp.route('/EOH_ZONA_TUR_DL', methods=['GET'])
def eoh_zona_tur_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, ccaa, residence, turistic_zone))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EOH/eoh_zona_tur.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=eoh_zona_tur" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=eoh_zona_tur" + file_type} 
