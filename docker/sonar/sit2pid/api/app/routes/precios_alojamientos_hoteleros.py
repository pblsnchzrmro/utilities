...
file_name = 'precios_alojamientos_hoteleros'
precios_alojamientos_hoteleros_bp = Blueprint('precios_alojamientos_hoteleros', __name__)

@precios_alojamientos_hoteleros_bp.route('/PRECIOS_ALOJAMIENTOS_HOTELEROS_DL', methods=['GET'])
def precios_alojamientos_hoteleros_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/MABRIAN/mabrian_precios.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Mabrian indices dataframe created, total rows {}...".format(df.shape[0]))

        # Definir el orden deseado de las columnas
        orden_columnas = ['AÑO', 'MES', 'CATEGORIA_ALOJAMIENTO', 'CCAA', 'PROVINCIA', 'PERIODO_ANTELACION', 'PRECIO_CHECK-IN_ENTRE_SEMANA', 'PRECIO_CHECK-IN_FIN_SEMANA']

        # Reordenar el DataFrame según el orden de las columnas
        df = df.reindex(columns=orden_columnas)
        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=precios_alojamientos_hoteleros" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=precios_alojamientos_hoteleros" + file_type}
