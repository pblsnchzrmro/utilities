...
file_name = 'puertos'
puertos_bp = Blueprint('puertos', __name__)

@puertos_bp.route('/PUERTOS_DL', methods=['GET'])
def puertos_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if port_auth != "Todos" and not "Todos" in port_auth:
            fields["AUT_PORTUARIA"] = port_auth
        if ccaa != "Todos" and not "Todos" in ccaa:
            fields["CCAA"] = ccaa

        df = read_blob_file("/PUERTOS/puertos.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "Puertos dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=puertos" + file_type}
