...
file_name = 'epa_ccaa'
epa_ccaa_bp = Blueprint('epa_ccaa', __name__)

@epa_ccaa_bp.route('/EPA_CCAA_DL', methods=['GET'])
def epa_ccaa_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/EPA/epa_ccaa.csv", fields, start_year, start_month, end_year, end_month)
        ...
        header = {"Content-Disposition": "attachment; filename=epa_ccaa" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=epa_ccaa" + file_type}
