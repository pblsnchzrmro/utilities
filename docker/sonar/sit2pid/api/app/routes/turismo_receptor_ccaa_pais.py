...
file_name = 'turismo_receptor_ccaa_pais'
turismo_receptor_ccaa_pais_bp = Blueprint('turismo_receptor_ccaa_pais', __name__)

@turismo_receptor_ccaa_pais_bp.route('/TURISMO_RECEPTOR_CCAA_PAIS_DL', methods=['GET'])
def turismo_receptor_ccaa_pais_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/TELEFONIA_TURISMO/turismo_receptor_ccaa_pais.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Telefonia turismo dataframe created, total rows {}...".format(df.shape[0]))

        df = df.sort_values(by=["AÑO", "MES", "CCAA_DESTINO", "CONTINENTE_ORIGEN", "PAIS_ORIGEN"])
        df  = df[["AÑO", "MES", "CCAA_DESTINO", "CONTINENTE_ORIGEN", "PAIS_ORIGEN", "TURISTAS", "PERNOCTACIONES", "ESTANCIA_MEDIA"]]
        
        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        
        header = {
            "Content-Disposition": "attachment; filename=turismo_receptor_ccaa_pais" + file_type}
        # header = {"Access-Control-Allow-Origin": "*","Access-Control-Expose-Headers": "content-disposition","Content-Disposition": "attachment; filename=turismo_receptor_ccaa_pais" + file_type}
