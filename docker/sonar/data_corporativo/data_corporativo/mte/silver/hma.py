...
df = spark.read.table("operaciones.bronze_mte.hMA")
...
df = df.withColumn("EsMercado", when(col("EsMercado") == True, 1).otherwise(0))
...
# Dataset Mercados con los distintos mercados
df_mercados = df.filter(col("EsMercado") == 1).select("Nombre", "FECHA_CARGA").distinct()
...
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('operaciones.silver_mte.hMA')

df_mercados.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('operaciones.silver_mte.Mercado')

# Save to PEOPLE catalog
df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('people.silver_mte.hMA')

df_mercados.write.mode("overwrite").format("delta").option("overwriteSchema", "true").option("mergeSchema", "true").saveAsTable('people.silver_mte.Mercado')