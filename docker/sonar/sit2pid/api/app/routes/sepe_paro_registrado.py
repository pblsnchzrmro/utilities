...
file_name = 'sepe_paro_registrado'
sepe_paro_registrado_bp = Blueprint('sepe_paro_registrado', __name__)

@sepe_paro_registrado_bp.route('/SEPE_PARO_REGISTRADO_DL', methods=['GET'])
def sepe_paro_registrado_dl():
    ...
    # registered_unemployment = request.args.get('Paro registrado')
    ...
    # registered_unemployment = "Todos" if registered_unemployment is None else registered_unemployment
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        # if registered_unemployment is None:
        #     return Response(
        #         "You must select a value for registered_unemployment!",
        #         status=500
        #     )
        if start_month:
        ...
        # fields["PARO_REGISTRADO"] = registered_unemployment

        df = read_blob_file("/SEPE/sepe_paro_registrado.csv", fields,
                            start_year, start_month, end_year, end_month)
        logging.info(
            "SEPE dataframe created, total rows {}...".format(df.size))

        df = df.astype({"NUM_PARO_REGISTRADO": int})

        df_bytes = get_df_bytes_csv(
            df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {
            "Content-Disposition": "attachment; filename=sepe_paro_registrado" + file_type}
