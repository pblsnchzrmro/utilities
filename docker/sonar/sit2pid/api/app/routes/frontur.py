...
file_name = 'frontur'
frontur_bp = Blueprint('frontur', __name__)

MIN_YEAR = 2015
MIN_MONTH = 10

@frontur_bp.route('/FRONTUR_DL', methods=['GET'])
def frontur_dl():
    ...
    country = request.args.get('País de residencia')
    traveler_type = request.args.get('Tipo de visitante')
    ccaa = request.args.get('CCAA de destino')
    file_type = request.args.get('Tipo de fichero')

    country = "Todos" if country is None else country
    traveler_type = "Todos" if traveler_type is None else traveler_type
    ccaa = "Todos" if ccaa is None else ccaa
    file_type = ".xlsx" if file_type is None else file_type

    logging.info("Parameters: {}, {}, {}, {}, {}, {}, {}...".format(start_month, start_year, end_month, end_year, country, traveler_type, ccaa))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields["TIPO_VISITANTE"] = traveler_type
        fields["CCAA_DESTINO"] = ccaa

        df = read_blob_file("/FRONTUR/frontur.csv", fields, start_year, start_month, end_year, end_month)
        df['VISITANTES'] = df['VISITANTES'].round().astype(int)
        logging.info("Frontur dataframe created, total rows {}...".format(df.size))

        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=frontur" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=frontur" + file_type}
