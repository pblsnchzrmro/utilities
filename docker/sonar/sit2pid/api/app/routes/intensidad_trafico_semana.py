...
file_name = 'intensidad_trafico_semana'
intensidad_trafico_semana_bp = Blueprint('intensidad_trafico_semana', __name__)

@intensidad_trafico_semana_bp.route('/INTENSIDAD_TRAFICO_SEMANA_DL', methods=['GET'])
def intensidad_trafico_semana_dl():
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        df = read_blob_file("/INTENSIDAD_TRAFICO/imd_semana.csv", fields, start_year, start_month, end_year, end_month)
        logging.info("Intensidad trafico semanal dataframe created, total rows {}...".format(df.shape[0]))

        df = df[["AÑO", "MES", "DIA_SEMANA", "CCAA", "PROVINCIA", "CARRETERA", "TIPO_ESTACION", "ESTACION", "IMD_VEHICULO_LIGERO", "IMD_VEHICULO_PESADO", "IMD_VEHICULO_TOTAL"]]
        
        df_bytes = get_df_bytes_csv(df) if file_type == ".csv" else get_df_bytes_xlsx(df)
        header = {"Content-Disposition": "attachment; filename=imd_semana" + file_type}
        # header = {'Access-Control-Allow-Origin': '*','Access-Control-Expose-Headers': 'content-disposition',"Content-Disposition": "attachment; filename=imd_semana" + file_type}
