...
file_name = 'ind_rentabilidad_zona_tur'
ind_rentabilidad_zona_tur_bp = Blueprint('ind_rentabilidad_zona_tur', __name__)

@ind_rentabilidad_zona_tur_bp.route('/IND_RENTABILIDAD_ZONA_TUR_DL', methods=['GET'])
def ind_rentabilidad_zona_tur_dl():
    ...
    logging.info("Parameters: {}, {}, {}, {}, {}...".format(
        start_month, start_year, end_month, end_year, turistic_zone))
    ...
    if (start_month and not start_year) or ((end_year or end_month) and not (start_year and start_month)) or not file_type in [".csv", ".xlsx"]:
        ...
        fields = {}
        if turistic_zone != "Todos" and not "Todos" in turistic_zone:
            fields["ZONA_TURISTICA"] = turistic_zone

        df = read_blob_file("/IND_RENTABILIDAD/ind_rentabilidad_zona_tur.csv", fields,
        ...
        header = {
            "Content-Disposition": "attachment; filename=ind_rentabilidad_zona_tur" + file_type}
